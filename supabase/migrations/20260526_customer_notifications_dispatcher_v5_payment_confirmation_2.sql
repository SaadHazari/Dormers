-- ============================================================================
-- Customer notifications dispatcher v5 — adapt payment_order_confirmed to
-- match the user-built `payment_confirmation_2` Meta template.
--
-- Two changes from v4 specific to payment_order_confirmed:
--   1. Language code: 'en_US' (template registered as English (US), not
--      plain English). Other templates remain on 'en'.
--   2. Structure: static header (no params) + positional body with two
--      vars — {{1}} = total formatted as "AED 89.50", {{2}} = plan name.
--      v4 sent named params keyed on first_name/plan_name/total_aed/
--      start_date, which Meta rejected with #132001 because the template
--      doesn't have those variables.
--
-- Meta template structure (registered):
--   Header: "Successful payment"       (static)
--   Body:   "Your payment of {{1}} for {{2}} has been processed
--           successfully.\n\nWelcome to the Club !"  (positional)
--   Footer: "Finance Team"             (static)
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Per-kind language code. payment_confirmation_2 was registered as
    -- English (US) in Meta Business Manager; everything else as English.
    lang_code := CASE notif_row.kind
    ...
    meta_payload := jsonb_build_object(
      ...
      'template', jsonb_build_object(
        'name', template_name,
        'language', jsonb_build_object('code', lang_code),
...
COMMENT ON FUNCTION public.dispatch_customer_notifications_tick() IS
  'v5: payment_order_confirmed switched to positional params + en_US language (matches the payment_confirmation_2 Meta template). Other kinds unchanged.';
