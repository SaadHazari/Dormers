-- ============================================================================
-- Welcome meal WhatsApp — new notification kind + dispatcher v9.
--
-- Phase 7 introduces the referee free meal as a real Subscription (planKind
-- = 'gift'). On claim, claimGift queues a `welcome_meal_confirmed`
-- notification — distinct from `payment_order_confirmed` so the messaging
-- voice is right ("Welcome!" not "Successful payment"). No email, no Zoho.
--
-- Two changes here:
--   1. Extend the customer_notifications.kind CHECK to include the new
--      'welcome_meal_confirmed' value.
--   2. Bump the dispatcher to v9 with a per-kind CASE branch + lang
--      pinning.
--
-- Manual ops step (outside this migration):
--   • Register a new Meta template `welcome_meal_confirmed` in
--     Business Manager. Suggested copy:
--        Header: "Welcome to Dormers, {{1}}!"   (named: first_name)
--        Body:   "Your free welcome meal is on its way.
--                 First delivery: {{2}}.
--                 We'll WhatsApp you when the rider is close."
--                                                  (named: start_date)
--   • Save the template name into the vault:
--        select vault.create_secret('<template_name>', 'tpl_welcome_meal_confirmed', '');
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    components := CASE notif_row.kind
      ...
      -- v9: welcome meal — named params, header carries first_name, body
      -- carries the first delivery date.
