-- ============================================================================
-- v6: referral_converted, refund_processed, subscription_ended
--
-- Three new customer notification kinds that close open loops:
--   • referral_converted — inviter hears their friend signed up + credit amount
--   • refund_processed   — customer hears their refund is on the way
--   • subscription_ended — warm wrap-up when the plan completes
--
-- Meta template param contract (all language 'en'):
--   referral_converted:  header {{name}}, body {{referral}} {{x}}
--   refund_processed:    header {{name}}, body {{x}}
--   subscription_ended:  header {{name}}, body {{plan_name}} {{x}}
--
-- App payload keys → dispatcher mapping:
--   referral_converted:  { referral, credit_aed }    → referral, x
--   refund_processed:    { refund_aed }              → x
--   subscription_ended:  { plan_name, delivered_meals } → plan_name, x
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- New payload fields for v6 kinds
    ...
    components := CASE notif_row.kind
      ...
      -- ── v6 kinds — header uses 'name' (not 'first_name') per Meta templates ──
