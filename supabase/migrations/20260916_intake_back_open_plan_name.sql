-- ============================================================================
-- The reopening template `intake_back_open` names the plan they were on.
--
-- The owner checked the approved template at Meta on 2026-09-16: besides
-- first_name in the header it takes {{plan_name}} in the body. The dispatcher
-- was sending a header only, which Meta rejects with a 400. The broadcast
-- route now sends the customer's most recent plan name alongside it, and
-- skips the WhatsApp entirely for anyone with no plan to name.
--
-- The body was copied from the applied season_reopen_notices migration; only
-- the marked branch differs.
--
-- Applied live to Dormers-Ohio (yjjayivwfqjfppawgyaz) through the Management
-- API as migration `intake_back_open_plan_name`. This file is the mirror.
-- ============================================================================
