-- ============================================================================
-- Customer notifications dispatcher v12 — subscription_renew_nudge CASE.
--
-- Renders the approved Meta template `subscription_renew_nudge` (locale `en`,
-- Utility category) with 3 named params:
--   • Header: first_name           (derived from customers.name)
--   • Body:   plan_name             (payload)
--   • Body:   end_date              (payload, formatted to "FMDDth FMMonth")
--
-- The "Renew Now" button is a STATIC URL (https://dormers.ae/dashboard/plan
-- ?renew=1) baked into the template at Meta approval time, so no button
-- component is included in the API payload.
--
-- Also extracts `end_date_str` from the payload alongside the existing
-- `start_date_str`, `meal_date_str`, `plan_name_str`, `total_aed_str` vars
-- so the new CASE branch has the formatted value to hand.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    components := CASE notif_row.kind
      ...
      -- v12: subscription_renew_nudge — T-3 days renewal reminder.
      -- Header carries first_name; body carries plan_name + end_date.
      -- The "Renew Now" button is a STATIC URL baked into the Meta template
      -- at approval time (https://dormers.ae/dashboard/plan?renew=1), so no
      -- button component is included here.
