-- ============================================================================
-- Customer notifications dispatcher v10 — payment_order_confirmed locale fix.
--
-- Meta returned 404 "Template name (payment_order_confirmed) does not exist
-- in en_US" for every send since v8 went live. The Meta-approved template is
-- registered under language "English" (code `en`), not `en_US`. v9 carried the
-- same wrong CASE branch from v8.
--
-- Only change vs v9: the payment_order_confirmed locale flips from 'en_US'
-- to 'en'. Everything else (welcome_meal CASE, component layout, kind CHECK)
-- is identical.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- v10: payment_order_confirmed flips en_US → en (template approved as "English")
