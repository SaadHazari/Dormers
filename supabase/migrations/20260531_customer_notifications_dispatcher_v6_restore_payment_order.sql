-- ============================================================================
-- Customer notifications dispatcher v6 — restore payment_order_confirmed.
--
-- Regression context: v5 (20260526_..._payment_confirmation_2.sql) added the
-- payment_order_confirmed branch with positional body params (AED total + plan
-- name) and the en_US language code. v4_lang_per_kind (20260527_...) was
-- written as an iteration ON TOP of v3 but landed AFTER v5 by timestamp; it
-- overwrote v5 and silently dropped both the components case AND the en_US
-- override.
--
-- Result: every payment confirmation since 2026-05-26 has been sent with
-- `components: null` and `language: en`. Meta rejects with #132001
-- "Template name (payment_confirmation_2) does not exist in en". The
-- dispatcher updates sent_at = now() regardless of Meta's response, so the
-- failure has been invisible — orders.whatsapp_sent_at reads as success
-- while the customer never receives the WhatsApp.
--
-- v6 restores v5's payment_order_confirmed branch on top of v4's
-- per-kind-language CASE, so both regressions are healed and the other
-- five kinds (skip/resume/pause confirms) are preserved.
-- ============================================================================
...
COMMENT ON FUNCTION public.dispatch_customer_notifications_tick() IS
  'v6: restore payment_order_confirmed branch (positional body params, en_US lang) that was dropped by v4_lang_per_kind. Keeps v4 per-kind language CASE for meal_resumed_confirm.';
