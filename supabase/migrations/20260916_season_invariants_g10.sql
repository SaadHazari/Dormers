-- ============================================================================
-- Season wind-down, plan G: the rest of the invariants (spec G10) and alert
-- discipline.
--
-- season_invariants_tick runs hourly. A standing breach used to be shouted
-- every hour; _season_alert_once now says each breach at most every six
-- hours (the time of the last alert lives in intake_settings.season_digest_state
-- under "alerted"). New checks: a delivery recorded after the close day; a
-- delivery after the wrap-up day on a plan without a buffer grant; a paid
-- season hold without its waitlist credit; a season-skip credit still pending
-- two days after its meal date.
--
-- The body was copied from the applied season_reopen_notices migration;
-- lines marked "Plan G" and the new blocks differ.
--
-- Applied live to Dormers-Ohio (yjjayivwfqjfppawgyaz) through the Management
-- API as migration `season_invariants_g10`. This file is the mirror.
-- ============================================================================

BEGIN;

-- Sends p_message unless the same key was alerted less than six hours ago;
-- returns the map with the key stamped when it did send.
CREATE OR REPLACE FUNCTION public._season_alert_once(p_alerted jsonb, p_key text, p_message text, p_button text)
RETURNS jsonb
LANGUAGE plpgsql
SET search_path = public
AS $$
DECLARE
  v_last timestamptz := NULLIF(p_alerted->>p_key, '')::timestamptz;
BEGIN
  IF v_last IS NOT NULL AND v_last > now() - interval '6 hours' THEN
    RETURN p_alerted;
  END IF;
  BEGIN
    PERFORM public.send_admin_whatsapp_alert(p_message, p_button);
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'season invariant alert failed: %', SQLERRM;
    RETURN p_alerted;
  END;
  RETURN COALESCE(p_alerted, '{}'::jsonb) || jsonb_build_object(p_key, now()::text);
END;
$$;
...
REVOKE EXECUTE ON FUNCTION public._season_alert_once(jsonb, text, text, text) FROM public, anon, authenticated;
