-- ============================================================================
-- Post-payment fan-out retry — hourly cron that picks up orders where any
-- of the three channels (WhatsApp / ZeptoMail / Zoho) failed and re-attempts
-- them via the internal /api/internal/post-payment-retry route. After 5
-- attempts on the same channel, fires a one-shot admin WhatsApp alert and
-- stops retrying for that order so we don't keep burning Zoho / ZeptoMail
-- calls into a permanent failure.
--
-- Vault secrets used (existing):
--   • whatsapp_access_token, whatsapp_phone_number_id (admin WhatsApp)
--   • whatsapp_admin_alert_template_name, admin_alert_phone_e164 (admin alert)
--   • admin_base_url (e.g. 'https://dormers.ae' — prepended to the retry path)
--
-- Vault secret to add (one-time, in Supabase Dashboard → Vault):
--   • internal_retry_secret — random 32+ char string. MUST match the value
--     of the INTERNAL_RETRY_SECRET env var in Netlify. The retry route
--     uses it to gate access; the cron passes it in the Authorization
--     header.
-- ============================================================================

BEGIN;

-- New column: one-shot admin alert tracker per order. Prevents alert spam
-- when an order has a permanently broken channel (e.g., bad email address
-- — ZeptoMail rejects forever). Once alerted, cron stops re-trying.
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS post_payment_admin_alerted_at timestamptz;
...
BEGIN
  ...
  IF base_url IS NULL OR retry_secret IS NULL THEN
    RAISE WARNING 'retry_post_payment_fanout_tick: required vault secrets missing (admin_base_url / internal_retry_secret)';
    no_config_total := 1;
    retried_count := 0;
    alerted_count := 0;
  ...
  FOR order_row IN
    ...
    LIMIT 50
  LOOP
    -- Count attempts per channel from the error log.
    attempts_whatsapp := 0;
    attempts_email := 0;
    attempts_zoho := 0;
    ...
    IF max_attempts >= 5 THEN
      -- Budget exhausted on at least one still-pending channel — admin alert.
      ...
      IF order_row.whatsapp_sent_at IS NULL THEN
        channels_failed := array_append(
          channels_failed,
          format('WhatsApp (%s tries)', attempts_whatsapp)
        );
      END IF;
      IF order_row.email_sent_at IS NULL THEN
        channels_failed := array_append(
          channels_failed,
          format('email (%s tries)', attempts_email)
        );
      END IF;
      IF order_row.zoho_invoice_id IS NULL THEN
        channels_failed := array_append(
          channels_failed,
          format('Zoho invoice (%s tries)', attempts_zoho)
        );
      END IF;
    ...
    ELSE
      -- Under budget — invoke the retry route. Idempotent end-to-end:
      -- the route's orchestrator skips channels that already succeeded.
      SELECT net.http_post(
...
COMMENT ON FUNCTION public.retry_post_payment_fanout_tick() IS
  'Hourly retry of post-payment fan-out. Re-invokes /api/internal/post-payment-retry per order; admin-alerts after 5 attempts on any pending channel and stops retrying.';

-- Schedule the cron at :15 past every hour. Offset from the dispatcher tick
-- (which runs every 5 min) and from the stale fraud queue alert (:05) so we
-- never overlap network bursts.
DO $$
BEGIN
  PERFORM cron.unschedule('retry_post_payment_fanout_hourly');
EXCEPTION
  WHEN OTHERS THEN
    NULL;
END $$;
...
SELECT cron.schedule(
  ...
  '15 * * * *',
  $cron$ SELECT public.retry_post_payment_fanout_tick(); $cron$
);
