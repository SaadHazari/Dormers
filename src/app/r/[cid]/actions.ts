...
// ── Conversion credit lives elsewhere, on purpose ─────────────────────────
// creditInviterOnConversion moved to
// src/contexts/referrals/usecases/credit-inviter.ts
//
// Every export in THIS file is a public HTTP endpoint, because the file opens
// with 'use server'. That payout function is server-to-server only, runs on the
// service-role client and moves money, so being exported from here made it an
// unauthenticated payout endpoint. Import it from the context module instead.
// Do not re-export it here.
