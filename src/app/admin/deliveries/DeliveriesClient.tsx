...
import { Truck, Pause, SkipForward } from 'lucide-react'
...
export function DeliveriesClient({ subscriptions }: Props) {
    ...
    // Group by dorm for the delivery view
    ...
    return (
        <div>
            ...
            {/* Grouped by dorm */}
            {dorms.map(([dorm, subs]) => (
                <div key={dorm} className="mb-5">
                    <div className={`flex items-center gap-2 mb-2`}>
                        <h2 className={`text-[13px] font-black ${t.heading}`}>{dorm}</h2>
                            ...
                            <tbody>
                                {subs.map(s => (
                                    <tr
                                        ...
                                        onClick={() => router.push(`/admin/customers/${s.customer_id}`)}
                                    >
                                        ...
                                        <td className={`px-3 py-2 ${t.body}`}>{s.meal_preference || '—'}</td>
                        ...
                        {subs.map(s => (
                            ...
                            >
                                ...
                                <div className={`text-[11px] ${t.muted}`}>
                                    {s.plan_name.replace(/-/g, ' ')} · {s.meal_preference} · {s.delivered_meals}/{s.total_meals} meals
                                </div>
                            </div>
                        ))}
                    </div>
