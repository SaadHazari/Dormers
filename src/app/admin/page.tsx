...
export default async function AdminOverviewPage() {
    ...
    const [kpiResult, cronResult, recentOrdersResult] = await Promise.allSettled([
    ...
    const kpis = kpiResult.status === 'fulfilled'
        ? (kpiResult.value.data as Record<string, number>) ?? EMPTY_KPIS
        : EMPTY_KPIS

    const cronJobs = cronResult.status === 'fulfilled'
        ? (cronResult.value.data as Array<Record<string, unknown>>) ?? []
        : []

    const recentOrders = recentOrdersResult.status === 'fulfilled'
        ? (recentOrdersResult.value.data ?? []) as Array<{
            id: string
            customer_id: string
            plan: string
            meals_count: number
