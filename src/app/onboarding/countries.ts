// Phone country codes — alphabetical by name. Covers every ISO 3166-1
// alpha-2 entry. Dial codes are E.164 prefixes; combined with a local
// number to form the canonical phone string we store in
// customers.whatsapp_number.
//
// NANP entries (US, Canada, Caribbean) all share +1 with longer prefixes
// (e.g. Anguilla +1264). splitPhone() does longest-prefix match so a stored
// "+1264..." resolves to Anguilla, while a bare "+1XXX..." falls through to
// the first +1 country alphabetically. That's fine — the dial code is what
// gets dialed; the chosen country just pre-populates the dropdown on edit.

export interface Country {
    code: string   // ISO 3166-1 alpha-2
    name: string
    dial: string   // E.164 prefix, e.g. "+971"
...
// UAE is the default selection on a fresh form (this is a UAE-based service).
// The list above stays alphabetical regardless.
export const DEFAULT_COUNTRY: Country =
    COUNTRIES.find(c => c.code === 'AE') ?? COUNTRIES[0]

// Split a stored E.164 string back into (country, localNumber).
// Falls back to the default + raw input if no prefix matches — safe for legacy
// rows that may not have the leading "+".
export function splitPhone(stored: string | null | undefined): {
    ...
    if (!cleaned.startsWith('+')) {
        return { country: DEFAULT_COUNTRY, local: cleaned.replace(/\D/g, '') }
    }
    // Match longest dial prefix first so "+1264" (Anguilla) wins over "+1" (US/CA),
    // and "+971" wins over "+97".
    const sorted = [...COUNTRIES].sort((a, b) => b.dial.length - a.dial.length)
    const match  = sorted.find(c => cleaned.startsWith(c.dial))
    if (!match) return { country: DEFAULT_COUNTRY, local: cleaned.replace(/\D/g, '') }
    return { country: match, local: cleaned.slice(match.dial.length).replace(/\D/g, '') }
}

// Combine a country dial code + local digits into canonical E.164.
// Strips a leading 0 from the local part (national trunk prefix common in
// AE/UK/IN: users type "0501234567" but E.164 expects "501234567").
