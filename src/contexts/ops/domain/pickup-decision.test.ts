...
function input(over: Partial<PickupInput> = {}): PickupInput {
  return { expectedTotal: 6, geminiCount: 6, attempt: 1, riderAsserted: false, ...over }
}
...
describe('the budget always terminates in a way forward', () => {
  it('no AI answer can leave the rider with nothing to do', () => {
    for (const geminiCount of [null, 0, 2, 99]) {
      ...
      // Either the day opened, or he is offered the assertion path.
      expect(d.accepted || d.outcome === 'needs_assertion' || d.outcome === 'uncountable').toBe(true)
    }
  })
})
